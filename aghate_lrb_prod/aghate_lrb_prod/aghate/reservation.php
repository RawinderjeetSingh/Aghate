...
if(strlen($id)>0)
{
	...
	$DeleteButton = '<input type="button" id="Supprimer" name="Supprimer" value="Supprimer" class="btn btn-danger" onclick="DelReservation(\''.$NomTableLoc.'\');"/>'; 	
		...
		<td>
			<table>
				...
				<tr>
					<td>
						 <div class="input-prepend input-append">
							...
							print '<span class="add-on"><b>Lit  </b>&nbsp;&nbsp;&nbsp;&nbsp;</span>';							
							print  $Html->InputSelect($ListRoom,"room_id",$room_id,"./commun/ajax/ajax_aghate_resa_autocomplet_service.php","class='input-small' $AttrRoom" );
				...
				<tr>
					<td align="center">
						<div class="noprint">
							<input type="button" id="Enregistrer" name="Enregistrer" value="Enregistrer" class="btn btn-success" onclick="SavePage('<?php print $Aghate->NomTableLoc;?>')"/>
							<?php
							print $DeleteButton;
							?>
						</div>
	...
	if (count($SejourInfo) > 0)
		...
		echo "<table class='table' border='1' align='center'>
				...
				<tr><th colspan='3'> NDA : ".$SejourInfo[0]['NODA']." / Gilda </th>
					<th colspan='3' align='right'>  Aghate <a href='#' ID='MAJ' >Mettre a jour</a></th>
		...
		for($i=0;$i < $TotRow; $i++)
		{
			if($SejourInfo[$i]['TYMVAD'] != 'SH')
			{
				...
				<tr>
					...
					<td>".$SejourInfo[$i]['DAMVAD']." ".$SejourInfo[$i]['HHMVAD']."</td>
					<td>".$SejourInfo[$i+1]['DAMVAD']." ".$SejourInfo[$i+1]['HHMVAD']."</td>
				 ";	
				...
				$ClassDtE= ($SejourInfo[$i]['DAMVAD']." ".$SejourInfo[$i]['HHMVAD']   !=$start_time)?'class=ClassRed':' ';
				$ClassDtS= ($SejourInfo[$i+1]['DAMVAD']." ".$SejourInfo[$i+1]['HHMVAD'] !=$end_time)?'class=ClassRed':' ';
