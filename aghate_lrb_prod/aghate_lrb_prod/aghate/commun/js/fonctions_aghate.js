...
function SavePage(TableLoc)
{
	...
	if (confirm(msg)==false) 
	  ...
	  res=LanceAjax("./commun/ajax/ajax_aghate_save_reservation.php",FormVariables+"&table_loc="+TableLoc);	
		result=res.split('|');
		//alert(res);
		if (result[1] != "OK"){
			alert(result[2]);
		...
		window.location.href = "../forms/reservation_aghate.php?id="+result[2]+"&table_loc="+TableLoc;
}
 
 
 
function DelReservation(TableLoc)
{
	var id    				=$("#id").val();
	...
	{
		res=LanceAjax("./commun/ajax/ajax_aghate_del_reservation.php","id="+id+"&table_loc="+TableLoc);	
