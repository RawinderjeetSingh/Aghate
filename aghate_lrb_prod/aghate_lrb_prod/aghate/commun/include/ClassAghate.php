...
class Aghate  extends MySQL
{
	...
	*/
	...
	function GetInfoEntry ($id){
		$this->CheckTableInit();
		$sql = "SELECT start_time, room_id,id_prog, name, description, end_time, protocole, uh,
					type , create_by, ".$this->NomTableLoc.".noip, medecin, nda, nom, prenom, ddn,sex
	...
	* Recupère la date fin programmé dans la table agt_prog
	...
	function GetDateFinProg($entry_id)
	{
		$sql = "SELECT end_time_prog FROM agt_prog WHERE id='".$entry_id."'";
		$res = $this->select($sql);
		if (!empty($res[0]['end_time_prog']))
			return $res[0]['end_time_prog'];
		else
			return false;
	}
...
//echo $sql;		
	...
	*/
	...
	{	
		...
		$sql_chk="SELECT ".$this->NomTableLoc.".*,service_id
					FROM  ".$this->NomTableLoc.",agt_room
					WHERE ".$this->NomTableLoc.".room_id=agt_room.id
					AND statut_entry != 'SUPPRIMER' 
					AND noip='$NIP' 
	...
	* ========================================================================*/
	...
	{
		$date_hr = array();
		//list($jour,$mois,$annee)=explode("/",$date_p); // mysql
		list($annee,$mois,$jour)=explode("-",$date_p); // oracle
