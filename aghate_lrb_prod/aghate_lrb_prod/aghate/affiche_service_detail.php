	...
	if ($key_lit == 'Panier')
	{
		...
		for ($dt = 0; $dt<count($dates); $dt++)
		{
			for ($h=0; $h < count($heures_ut)-1; $h++)
			{
				...
				//$UrlView=$ModuleReservationView."?id=".$_pat['entry_id'];
				//$lien = "<a href='#?'  onClick=\"OpenPopupResa('".$UrlView."')\"   title=\"".$title."\" >";
				
				// pour modifier le resa
				$EditLink=$ModuleReservationEdit."?area=".$service_id."&room=".$PanierId."&year=".$ResaYear."&month=".$ResaMonth."&day=".$ResaDay."&page=day";
				...
				$EditLink=$ModuleReservationEdit."?area=".$service_id."&room=".$PanierId."&year=".$ResaYear."&month=".$ResaMonth."&day=".$ResaDay."&hour=".$heures_ut[$h]."&page=day";
