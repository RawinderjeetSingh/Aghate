...
<table width="617" border="0" align="center" cellpadding="1" cellspacing="1">
  ...
  <tr>
    <td align="right">  Recherche par Patient : </td>
    <td><input type="text" name="nom" id="nom" value="<?php print $nom?>" /> 
      NIP : 
        <input type="text" name="nip" id="nip"  value="<?php print $nip?>"/></td>
  </tr>
  <tr>
...
<?php
	...
	if (($nom)&& (strlen($nom) > 2))$sql_nom=" and  agt_pat.nom like ('%".$_POST["nom"]."%')";
...
// Recherche des infos liée à la réservation
	$sql = "SELECT agt_pat.noip, 	nom, 	prenom ,	nomjf, 	ddn, 	sex,
		       ...
		       (agt_loc.end_time - agt_loc.start_time) as duration, 
		       agt_loc.start_time,	
		       agt_loc.end_time,		
		       agt_service.id,				
...
?>
	<table width="100%" border="1" cellspacing="1" cellpadding="1">
	  ...
	  if ($res) {
		  for ($i = 0; $i<$nb_info; $i++) {
		  	$duree_min= $row[$i]['duration']/60;// en minutes
		  	...
		  	//noip 	nom 	prenom 	nomjf 	ddn 	sex
				$UrlEdit=$ModuleReservationEdit."?area=".$area."&room=".$room."&hour=".$hour."minute=".$minute."&year=".$year."&month=".$month."&day=".$day."&page=day";
				...
				$link="OpenPopupResa('".$ModuleReservationView."?id=".$loc_id."')";						  	
		    ?>
