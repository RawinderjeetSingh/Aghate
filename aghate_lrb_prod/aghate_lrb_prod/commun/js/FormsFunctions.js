...
function getLink(id,balise){
	window.location.href="reservation_aghate.php?id="+id+"&filename="+balise.value+"&mode=Modifier&table_loc="+$("#table_loc").val();
}
function formsView(){
	var id = $("#name").val();
...
function returnView(file,id){
	var variables="filename="+file;
	res=LanceAjax("../commun/ajax/retourVue.php",variables);
	if (res=="KO")alert("Erreur id veuillez reessayer plus tard..");
	if(res.length>0){
		window.location ="reservation_aghate.php?id="+id+"&file="+file+"&name=LBL_forms"+res+"&table_loc="+$("#table_loc").val();
	}
	else{
		window.location ="reservation_aghate.php?id=38&file=Reservation&table_loc="+$("#table_loc").val();
	}   		
}
//=================================================================
// document ready
