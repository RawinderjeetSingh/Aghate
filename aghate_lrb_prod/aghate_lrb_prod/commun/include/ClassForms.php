...
class Forms extends MySQL
{
	...
	function GetForm($VID){
			$sql="select distinct val from forms where var='form' and idref=$VID order by val asc";
			$Res=$this->select($sql);
			return $Res;
	}
	...
	function PrintForm($DP,$VID,$mode_ref=""){
		$FichierRef='../forms/ListeFormulaires';
		$fp = fopen($FichierRef, 'rb');
		$aLines = file($FichierRef);
		$count=0;
		foreach($aLines as $sLine) {  
		...
		if(strlen($mode_ref) > 0)
			{
				echo "<a href='reservation_aghate.php?mode=Modifier&id=$VID&file=".$_GET['filename']."&table_loc=".$_GET['table_loc']."'>Modifier le formulaire</a><br/>";
				$compt=0;
				foreach ($formsr as $key => $val){
					foreach ($FormsList as $key1 => $val1){
						if($val['val']==$key1){
							$name="LBL_forms".$compt;
							$listResultat[]="reservation_aghate.php?&name=$name&id=$VID&filename=$key1|$val1";
							$compt++;
						}
					}
				}
		...
		{		
				echo "<a href='#' onclick=\"returnView('".$_GET['filename']."','".$VID."')\">Retour au mode vue</a>";
				echo "<br><br>Liste des formulaires  ".$this->Html->InputSelectFromArray($FormsList,'file',$_GET['filename'],"onchange='getLink(".$VID.",this)'").'<br>';
		}
		$n=0;
		echo '<table width="700">';
				...
				elseif($name!=''){
					...
					elseif( $DetailsForm[$DP['TYPE']][$i]=='longtext'){
							echo '<tr><td><b>'.$DetailsForm[$DP['LIBELLE']][$i];
							 	if($DetailsForm[$DP['COMPDATA']][$i]=='o')echo '*';
							 echo '</b><br>';
							echo "<textarea  id='".$name."' type='textarea' onblur=\"updateForms('".$VID."','".$name."','".$VID."','','".$DetailsForm[$DP['SOURCE']][$i]."','".$DetailsForm[$DP['ABV']][$i]."','".$DetailsForm[$DP['LIBELLE']][$i]."','".$DetailsForm[$DP['COMPDATA']][$i]."')\" class=\"form-control\" rows=\"10\" cols=\"93\"  ".$mode."/>".$value."</textarea></td></tr>";
						}
